...
float plane( in vec3 p ) {
    ...
    return dot( p, norm )-smoothstep(0.05, 1.0, sin(Time*0.0954-2.5248)*0.5+0.5)*6.0;
}
...
void main() {
    ...
    vec2 uv = gl_FragCoord.xy / ScreenResolution.xy * 2.0 - 1.0;
	...
	vec2 mouse=(MousePosition.xy / ScreenResolution.xy - 0.5) * 0.5;
	// if (MousePosition.z < 1.0) mouse = vec2(0.0);

	mat2 rotxz = rot(sin(Time*0.0652-0.5)*0.8+mouse.x*5.0+2.5);
